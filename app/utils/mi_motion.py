...
class MiMotionRunner:
    def __init__(self, task_value: dict):
        ...
        if ("+86" in mi_user) or "@" in mi_user:
            user = mi_user
        else:
            user = "+86" + mi_user
        if "+86" in user:
        ...
        self.log_str += f"创建虚拟ip地址：{self.fake_ip_headers}\n"
    ...
    def upload_step(self, userid, app_token):
        t = get_time()
        ...
        if response.get('message', '') == 'success':
            return f"修改步数（{self.step_count}）[success]", True
        else:
            return f"修改步数失败（{response}）", False

    # 主函数
    def login_and_post_step(self):
        # message, login_token, userid, app_token, status, cache
        if self.invalid:
        ...
        if '' in (self.login_token, self.userid, self.app_token):
            ...
            message, status = self.upload_step(self.userid, self.app_token)
...
if __name__ == "__main__":
    ...
    print(MiMotionRunner(task_value).login_and_post_step())
