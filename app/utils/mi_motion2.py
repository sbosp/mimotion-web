...
class MiMotion():
    ...
    def main(self):
        user = str(self.check_item.get("mi_user"))
        password = str(self.check_item.get("mi_password"))
        min_step = math.ceil(int(self.check_item.get("min_step", 10000)))
        max_step = math.ceil(int(self.check_item.get("max_step", 19999)))
        self.step_count = str(random.randint(min_step, max_step))

        # 获取token信息
        login_token = str(self.check_item.get("login_token", ""))
        userid = str(self.check_item.get("userid", ""))
        app_token = str(self.check_item.get("app_token", ""))

        print(self.check_item)
        print(login_token, userid, app_token)
        # 检查是否需要重新登录
